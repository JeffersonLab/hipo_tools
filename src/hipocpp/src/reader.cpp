...
namespace hipo {
   ...
   */
  reader::reader(bool ra) : isRandomAccess(ra) {
    printWarning();
    hipoutils.printLogo();
  }
  ...
  void reader::readRecordIndex() {
    ...
    while (positionOffset + 56 < hipoFileSize) {
      ...
      recIndex.fPosition = positionOffset;
      recIndex.fLength   = *(reinterpret_cast<int*>(&recheader[0]));
      recIndex.fEvents   = *(reinterpret_cast<int*>(&recheader[12]));
      ...
      recIndex.fDataLengthCompressed = compressWord & 0x0FFFFFFF;
      // recIndex.compressionType            = (compressWord&0xF0000000)>>28;

      if (magic_number == 0x0001dac0) {
        recIndex.fLength = __builtin_bswap32(recIndex.fLength);
        recIndex.fEvents = __builtin_bswap32(recIndex.fEvents);
        compressWord     = __builtin_bswap32(compressWord);
        version          = __builtin_bswap32(version);
      }

      inReaderIndex.addSize(recIndex.fEvents);
      ...
      positionOffset += recIndex.fLength * 4;
      inputStream.seekg(positionOffset, std::ios::beg);
  ...
  void reader::readRecord(hipo::record& record, int index) {
    long position = recordIndex[index].fPosition;
