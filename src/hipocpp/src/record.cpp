...
// LZ4_uncompress(buffer,destUnCompressed,decompressedLength);
  ...
  std::vector<char> record::build() {
    ...
    // System.arraycopy(recordHeader.getRecordHeaderData(), 0, buffer, 0,
    // recordHeader.getRecordHeaderLength()); System.arraycopy(indexBytes, 0, buffer,
    //                 recordHeader.getRecordHeaderLength() + recordHeader.getHeaderSize(),
    //                 indexBytes.length);

    // int position = recordHeader.getRecordHeaderLength() + recordHeader.getHeaderSize() +
    // indexBytes.length;
